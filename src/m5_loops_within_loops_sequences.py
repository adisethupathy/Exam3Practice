    ...
    for j in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j][k]) == int:
                list += [sequence_of_sequences[j][k]]
...
def run_test_big_letters():
    ...
    # Test 1:
    expected = 'OTSASOOPSAPSBOSCOPDOO'
    answer = big_letters([(3, 1, 4),  # not a string
                          'Ok what is ThiSAS?',  # OTSSS
                          (10, 'Ok what is THiSSS?', 10),  # not a string
                          [],  # not a string
                          ['oops'],  # not a string
                          'oops',  #
    ...
    for j in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j]) == str:
                if sequence_of_sequences[j][k].isupper() == True:
                    string += str(sequence_of_sequences[j][k])
    return string
